 ...
 * @param allEmotions - Array of { type: string; typeEn: string; confidence: number }
 * @returns { score: number; label: string; level: 'low' | 'normal' | 'high' | 'dangerous' }
 */
export const calculateStressIndex = (
  bpm: number,
  allEmotions: Array<{ type: string; typeEn: string; confidence: number }>
) => {
  // 1. BPM Component (Non-linear scaling around baseline 72)
  ...
  if (Array.isArray(allEmotions)) {
    allEmotions.forEach(emotion => {
      const typeEn = (emotion.typeEn || "").toUpperCase();
  ...
  // 3. Final Calculation
  // Balanced combination of BPM and Emotion
  ...
  let label = "Normal";
  let level: 'low' | 'normal' | 'high' | 'dangerous' = "normal";
